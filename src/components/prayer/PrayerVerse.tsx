...
const PrayerVerse: React.FC<PrayerVerseProps> = ({ verse, commonPrayers }) => {
  const renderTextWithCommonPrayers = (text: string) => {
    if (!commonPrayers) return text;
    
    // Check if the text matches a common prayer name
    const trimmedText = text.trim();
    const commonPrayer = commonPrayers[trimmedText];
    ...
    const sortedNames = [...commonPrayerNames].sort((a, b) => b.length - a.length);
